...
df = pd.read_csv("bart_augment.csv")
print(len(df))
...
for i in range(len(df)):
    train_text[i] = " ".join(train_text[i].split()[:300])
...
with open('Pegasus_cnn_augmented_50epochs_350.csv', 'a', newline ='') as file:
	for i in range(len(tab)):
          try:
